...
import { createBackupFiles, isConfigured, isValidSession, json, sanitizeContentData, selectStorage, type AuditEntry } from "./_shared";
...
export async function onRequestPost(context: { request: Request; env: Env }): Promise<Response> {
  ...
  try {
    ...
    const withAudit = await storage.fetchContentWithAudit(env);
    ...
    await storage.saveContent(env, sanitizedData, audit, extraFiles, deleteFiles);
    const latest = await storage.fetchContent(env);
